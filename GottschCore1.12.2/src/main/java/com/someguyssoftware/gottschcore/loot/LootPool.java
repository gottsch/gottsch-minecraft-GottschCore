...
//			Treasure.logger.debug("creating roll for loot entry -> {}", lootentry.entryName );
	...
	// TODO: Allow modifications of conditions? If so need a way to uniquely
	// identify them.
	// ======================== FORGE END
	// ===============================================

	public static class Serializer implements JsonDeserializer<LootPool>, JsonSerializer<LootPool> {
		public LootPool deserialize(JsonElement element, Type p_deserialize_2_, JsonDeserializationContext p_deserialize_3_) throws JsonParseException {
			JsonObject jsonobject = JsonUtils.getJsonObject(element, "loot pool");
			String name = LootTableManager.readPoolName(jsonobject);
			LootEntry[] alootentry = (LootEntry[]) JsonUtils.deserializeClass(jsonobject, "entries", p_deserialize_3_, LootEntry[].class);
			LootCondition[] alootcondition = (LootCondition[]) JsonUtils.deserializeClass(jsonobject, "conditions", new LootCondition[0], p_deserialize_3_,
					LootCondition[].class);
