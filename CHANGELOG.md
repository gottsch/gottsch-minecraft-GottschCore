...
- **A decoration palette entry may name an ENTITY instead of a block** (`WeightedGrowth`, formerly `WeightedBlock`). `{ "entity": "dungeons2:shrieker", "weight": 1 }` grows a mob where a mushroom would go. Some things that read as scenery are mobs — GMM's Shrieker and Violet Fungus root themselves and never move — and this lets a datapack treat them as what they look like.
  - An entry names **exactly one** of `block` / `entity`; both or neither is a load error rather than a guess.
  - Spawning happens in `finalizeProcessing`, which receives a real `ServerLevelAccessor`, and is clipped to `StructurePlaceSettings#getBoundingBox` — the chunk box during a piece placement, and the same exactly-once guard vanilla's own `StructureTemplate.placeEntities` applies. `processBlockInfos` is NOT chunk-clipped and a piece is placed once per chunk it spans, so without that clip a mob would be spawned once per overlapping chunk. The box is **null unless someone set it**, which is the honest signal for a single-shot caller; there, everything spawns.
  - Only `floor_growth` / `hanging_growth` / `floating_growth` can place an entity. The oriented rules (`cobwebs`, `corner_cobwebs`, `wall_growth`, `underwater_growth`) derive a block *state* from geometry — a facing, a half — which an entity has no equivalent of, so an entity in one of those palettes is refused with a warning rather than guessed at.
- **Weighted decoration palettes** (`WeightedGrowth`). `DecorationRule` and `WallGrowthRule` `blocks` lists now accept either a bare block id or `{"block": <id>, "weight": <int>}`, mixed freely in one list. A bare id means weight 1, so **every palette written before this keeps working unchanged and nothing needs migrating**; encoding round-trips weight 1 back to a bare id so re-serialising a pack does not rewrite it into the verbose form.
