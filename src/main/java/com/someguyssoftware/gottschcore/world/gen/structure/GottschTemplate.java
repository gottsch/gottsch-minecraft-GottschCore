...
public class GottschTemplate extends Template {
	 ...
	 */
	public boolean placeInWorld(IServerWorld serverWorld, BlockPos pos, BlockPos pos2, PlacementSettings placement, final Block NULL_BLOCK, Map<BlockState, BlockState> replacementBlocks, Random random, int flags) {
		if (this.palettes.isEmpty()) {
			return false;
		} else {
			...
			if ((!blockInfoList.isEmpty() || !placement.isIgnoreEntities() && !this.entities.isEmpty()) && this.size.getX() >= 1 && this.size.getY() >= 1 && this.size.getZ() >= 1) {
				...
				for(GottschTemplate.BlockInfo blockInfo : processBlockInfos(serverWorld, pos, pos2, placement, blockInfoList, this)) {
					...
					if ((boundingBox == null || boundingBox.isInside(pos)) && processedBlock != NULL_BLOCK) {
						...
						if (blockInfo.nbt != null) {
							...
							serverWorld.setBlock(blockPos, Blocks.BARRIER.defaultBlockState(), 20);
						}
						...
						if (serverWorld.setBlock(blockPos, processedBlockState, flags)) {
							...
							if (blockInfo.nbt != null) {
								TileEntity tileentity1 = serverWorld.getBlockEntity(blockPos);
								if (tileentity1 != null) {
							...
							if (fluidstate != null && processedBlockState.getBlock() instanceof ILiquidContainer) {
								((ILiquidContainer)processedBlockState.getBlock()).placeLiquid(serverWorld, blockPos, processedBlockState, fluidstate);
				...
				while(flag && !list1.isEmpty()) {
					...
					while(iterator.hasNext()) {
						...
						FluidState fluidstate2 = serverWorld.getFluidState(blockpos2);

						for(int k1 = 0; k1 < adirection.length && !fluidstate2.isSource(); ++k1) {
							BlockPos blockpos1 = blockpos3.relative(adirection[k1]);
							FluidState fluidstate1 = serverWorld.getFluidState(blockpos1);
							if (fluidstate1.getHeight(serverWorld, blockpos1) > fluidstate2.getHeight(serverWorld, blockpos3) || fluidstate1.isSource() && !fluidstate2.isSource()) {
						...
						if (fluidstate2.isSource()) {
							...
							if (block instanceof ILiquidContainer) {
								((ILiquidContainer)block).placeLiquid(serverWorld, blockpos2, blockstate2, fluidstate2);
								flag = true;
								iterator.remove();
							}
						}
				...
				if (i <= l) {
					if (!placement.getKnownShape()) {
						...
						updateShapeAtEdge(serverWorld, flags, voxelshapepart, l1, i2, j2);
					}

					for(Pair<BlockPos, CompoundNBT> pair : list2) {
						BlockPos blockpos4 = pair.getFirst();
						if (!placement.getKnownShape()) {
							BlockState blockstate1 = serverWorld.getBlockState(blockpos4);
							BlockState blockstate3 = Block.updateFromNeighbourShapes(blockstate1, serverWorld, blockpos4);
							if (blockstate1 != blockstate3) {
								serverWorld.setBlock(blockpos4, blockstate3, flags & -2 | 16);
							}

							serverWorld.blockUpdated(blockpos4, blockstate3.getBlock());
						}

						if (pair.getSecond() != null) {
							TileEntity tileentity2 = serverWorld.getBlockEntity(blockpos4);
							if (tileentity2 != null) {
				...
				if (!placement.isIgnoreEntities()) {
					this.addEntitiesToWorld(serverWorld, pos, placement);
				}
	 ...
	 * Adds blocks and entities from this structure to the given world.
	 */
	public boolean placeInWorld(IServerWorld serverWorld, BlockPos blockPos, BlockPos pos2, PlacementSettings placement, IDecayProcessor decayProcessor, 
		...
		if (this.palettes.isEmpty()) {
			...
			if ((!blockInfoList.isEmpty() || !placement.isIgnoreEntities() && !this.entities.isEmpty()) && this.size.getX() >= 1 && this.size.getY() >= 1 && this.size.getZ() >= 1) {
				...
				for(GottschTemplate.BlockInfo blockInfo : processBlockInfos(serverWorld, blockPos, pos2, placement, blockInfoList, this)) {
					BlockPos blockpos = blockInfo.pos;
					//================= GottschCore =================//
					// replace block with null block if it is a marker block
					Block processedBlock = blockInfo.state.getBlock();
				...
				List<DecayBlockInfo> decayBlockInfoList = decayProcessor.process(serverWorld, new Random(), transformedSize,
						NULL_BLOCK);
				...
				if (processed.nbt != null) {
					TileEntity tileentity = serverWorld.getBlockEntity(decayPos);
					IClearable.tryClear(tileentity);
					serverWorld.setBlock(decayPos, Blocks.BARRIER.defaultBlockState(), 20);
				}

				FluidState fluidstate = placement.shouldKeepLiquids() ? serverWorld.getFluidState(decayPos) : null;
				if (serverWorld.setBlock(decayPos, decay.getState(), flags)) {
					...
					if (processed.nbt != null) {
						TileEntity tileentity1 = serverWorld.getBlockEntity(decayPos);
						if (tileentity1 != null) {
					...
					if (fluidstate != null && decay.getState().getBlock() instanceof ILiquidContainer) {
						((ILiquidContainer)decay.getState().getBlock()).placeLiquid(serverWorld, decayPos, decay.getState(), fluidstate);
						if (!fluidstate.isSource()) {
							list1.add(decayPos);
						}
					}
			...
			if (!placement.isIgnoreEntities()) {
				this.addEntitiesToWorld(serverWorld, blockPos, placement);
			}
...
private ListNBT newDoubleList(double... doubles) {
	ListNBT listNbt = new ListNBT();
