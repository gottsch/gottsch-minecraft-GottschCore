...
public class GottschTemplate extends Template {
	...
	private final List<List<GottschTemplate.BlockInfo>> blocks = Lists.newArrayList();
	 ...
	 * @return
	 */
	public List<GottschTemplate.BlockInfo> addBlocksToList(BlockPos pos, PlacementSettings placement, Block block) {
	 ...
	 * @return
	 */
	public List<GottschTemplate.BlockInfo> addBlocksToList(BlockPos pos, PlacementSettings placement, Block block, boolean transformFlag) {
		List<GottschTemplate.BlockInfo> blockList = Lists.newArrayList();
		MutableBoundingBox boundingBox = placement.getBoundingBox();

		for(GottschTemplate.BlockInfo blockInfo : placement.func_227459_a_(this.blocks, pos)) {
			...
			if (boundingBox == null || boundingBox.isVecInside(blockPos)) {
				...
				if (blockState.getBlock() == block) {
					blockList.add(new GottschTemplate.BlockInfo(blockPos, blockState.rotate(placement.getRotation()), blockInfo.nbt));
				}
			}
		}
		return blockList;
...
//						GottschCore.LOGGER.debug("processedBlock -> {}", processedBlockState.getBlock().getRegistryName());
						...
						if (worldIn.setBlockState(blockPos, processedBlockState, flags)) { // this is where it adds to the world the original block
							...
							if (fluidState != null && processedBlockState.getBlock() instanceof ILiquidContainer) {
								((ILiquidContainer)processedBlockState.getBlock()).receiveFluid(worldIn, blockPos, processedBlockState, fluidState);
...
//					}
	...
	public static List<GottschTemplate.BlockInfo> processBlockInfos(@Nullable GottschTemplate template, IWorld worldIn, BlockPos offsetPos, PlacementSettings placementSettingsIn, List<GottschTemplate.BlockInfo> blockInfos) {
		List<GottschTemplate.BlockInfo> list = Lists.newArrayList();

		for(GottschTemplate.BlockInfo template$blockinfo : blockInfos) {
			BlockPos blockpos = transformedBlockPos(placementSettingsIn, template$blockinfo.pos).add(offsetPos);
			GottschTemplate.BlockInfo blockInfo = new GottschTemplate.BlockInfo(blockpos, template$blockinfo.state, template$blockinfo.nbt);
	...
	public static List<GottschTemplate.EntityInfo> processEntityInfos(@Nullable GottschTemplate template, IWorld worldIn, BlockPos offsetPos, PlacementSettings placementSettingsIn, List<GottschTemplate.EntityInfo> blockInfos) {
		List<GottschTemplate.EntityInfo> list = Lists.newArrayList();

		for(GottschTemplate.EntityInfo entityInfo : blockInfos) {
	...
	private void addEntitiesToWorld(IWorld worldIn, BlockPos offsetPos, PlacementSettings placementIn, Mirror mirrorIn, Rotation rotationIn, BlockPos centerOffset, @Nullable MutableBoundingBox boundsIn) {
		for(GottschTemplate.EntityInfo template$entityinfo : processEntityInfos(this, worldIn, offsetPos, placementIn, this.entities)) {
	...
	public CompoundNBT writeToNBT(CompoundNBT nbt) {
		if (this.blocks.isEmpty()) {
			...
			List<GottschTemplate.BasicPalette> list = Lists.newArrayList();
			GottschTemplate.BasicPalette template$basicpalette = new GottschTemplate.BasicPalette();
			list.add(template$basicpalette);

			for(int i = 1; i < this.blocks.size(); ++i) {
				list.add(new GottschTemplate.BasicPalette());
			}

			ListNBT listnbt1 = new ListNBT();
			List<GottschTemplate.BlockInfo> list1 = this.blocks.get(0);

			for(int j = 0; j < list1.size(); ++j) {
				GottschTemplate.BlockInfo template$blockinfo = list1.get(j);
				CompoundNBT compoundnbt = new CompoundNBT();
				...
				for(int l = 1; l < this.blocks.size(); ++l) {
					GottschTemplate.BasicPalette template$basicpalette1 = list.get(l);
			...
			if (list.size() == 1) {
				...
				for(GottschTemplate.BasicPalette template$basicpalette2 : list) {
	 ...
	 */
	public void read(CompoundNBT compound, List<Block> markerBlocks, Map<BlockState, BlockState> replacementBlocks) {
		this.blocks.clear();
		this.entities.clear();
		...
		if (compound.contains("palettes", 9)) {
			...
			for(int i = 0; i < listnbt2.size(); ++i) {
				this.readPalletesAndBlocks(listnbt2.getList(i), listnbt1, markerBlocks, replacementBlocks);
			}
		} else {
			this.readPalletesAndBlocks(compound.getList("palette", 10), listnbt1, markerBlocks, replacementBlocks);
		}
		...
		for(int j = 0; j < listnbt5.size(); ++j) {
			...
			if (compoundnbt.contains("nbt")) {
				...
				this.entities.add(new GottschTemplate.EntityInfo(vec3d, blockpos, compoundnbt1));
			}
		}
	}
	 ...
	 * @param blocksNBT
	 */
	private void readPalletesAndBlocks(ListNBT palletesNBT, ListNBT blocksNBT, List<Block> markerBlocks,
